...
function filterByCondition(arr, condition) {
  ...
  let filtArr = arr.filter((value) => value % 2 === 0);
  return filtArr;
}
...
function getElementAtIndex(arr, index) {
  ...
  if (index < 0 || index > arr.length) {
    return "індекс !знаходиться в межах довжини масиву"
  }
  return arr.at(index);
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (0 <= target && target<= arr.length && 0 <= start && start <= arr.length && 0 <= end && end<= arr.length) {
...
function customShift(arr) {
  ...
  const shiftedElement = arr.shift(0);
...
function customUnshift(arr, ...elements) {
  ...
  // та циклу for, початкове значення лічильника на 1 менше ніж довжина масива,
...
function customAt(arr, index) {
  ...
  if (typeof arr.at(index) === "number") {
    return console.log('Елемент є числом');
  }
